...
export const LOCALAI_MODULE_REGISTRY: LocalAiModuleContract[] = [
  ...
  },
  {
    ...
    summary: 'Agent Studio, Workflow Studio, demo run records, run controls, MCP allowlist checks, and timeline evidence exist; risky external tool approval and full workflow publishing remain incomplete.',
    ...
    completedCapabilities: ['Agent CRUD shell', 'Demo execution persistence', 'Workflow template/run path', 'Pause/cancel/retry/resume records', 'MCP allowlist evaluation'],
    incompleteCapabilities: ['Visual workflow canvas', 'Publish/rollback flow', 'External MCP tool discovery', 'Risky tool approval modal', 'Workflow import/export'],
  },
  {
    ...
    summary: 'Prompt Lab, Knowledge Base, Shared Memory Hub, context/recovery packs, knowledge asset summary, document preview, retrieval test, and redaction flows exist; full embedding-backed RAG indexing remains incomplete.',
    routes: ['/knowledge', '/memory', '/prompts'],
    ipcChannels: ['memory:list', 'memory:create', 'memory:update', 'memory:import', 'memory:generateContext', 'knowledge:assets:summary', 'knowledge:document:preview', 'knowledge:retrieval:test'],
    ...
    completedCapabilities: ['Prompt CRUD/versioning path', 'Memory lifecycle and filters', 'Context pack preview', 'Knowledge Base route', 'Knowledge asset summary', 'Knowledge preview/retrieval tests', 'Secret redaction coverage'],
    incompleteCapabilities: ['Embedding provider binding', 'Persistent vector index', 'RAG quality evaluation', 'Asset graph'],
  },
  {
    id: '06-observability',
    ...
    summary: 'Token Center, Diagnostics, Log Analyzer, usage records, observability report export, trace detail list, mock evaluation, and feedback records exist; full eval dataset/red-team loops are incomplete.',
    ...
    auditEvents: ['observability.report.generate', 'eval.mock.completed', 'feedback.created'],
    ...
    status: 'partial',
    completionPercent: 70,
    completedCapabilities: ['Usage summaries', 'Diagnostics panel', 'Trace detail list', 'Log analyzer', 'Mock evaluation', 'Redacted observability report', 'Report export UI'],
    incompleteCapabilities: ['Dedicated trace detail route', 'Evaluation dataset CRUD', 'Red-team suite', 'Closed-loop feedback automation'],
  },
  {
    id: '07-security-ops',
    name: 'Identity Security Audit and Ops',
    buildPlanPath: 'docs/build-plans/07-identity-security-audit-and-ops/build-plan.md',
    summary: 'Login, first-password-change, admin users, RBAC/ACL, audit hash/export, security report, backup/restore preview, merge-only restore apply, settings, and shortcut verification are locally implemented.',
    routes: ['/security', '/admin/users', '/admin/audit', '/settings', '/diagnostics'],
    ipcChannels: ['auth:login', 'auth:changePassword', 'user:list', 'audit:list', 'audit:export', 'security:report:generate', 'ops:backup:preview', 'ops:restore:preview', 'ops:restore:apply'],
    permissions: ['admin:users', 'admin:audit', 'ops:backup', 'ops:restore', 'settings:read', 'settings:write'],
    auditEvents: ['auth.login.succeeded', 'auth.password.changed', 'permission.denied', 'audit.exported', 'ops.backup.previewed', 'ops.restore.previewed', 'ops.restore.applied'],
    ...
    status: 'partial',
    completionPercent: 82,
    completedCapabilities: ['Local login/session', 'Admin users', 'RBAC/ACL guards', 'Tamper-evident audit export', 'Backup and restore preview', 'Merge-only restore apply', 'Theme/language settings'],
    incompleteCapabilities: ['Secret rotation', 'Migration runner', 'Crash/data repair tool', 'Full diagnostics export workflow'],
  },
];
